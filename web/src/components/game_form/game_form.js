...
class GameForm extends React.Component {
  ...
  handleSelectProvince(event) {
    ...
    _this.setState({provinceid: val,provincename: text, collegeid: 0, collegename:'无限制', instituteid: 0, institutename: '无限制'});
  ...
  handleSelectCollege(event) {
    ...
    var val = event.target.value;
    var text = event.target.options[event.target.selectedIndex].text;
    _this.setState({collegeid: val,collegename: text, instituteid: 0, institutename:'无限制'});
    var arr = [{'key':0,'val':'无限制'}];
    if(val > 0){
